...
def get_yolo_metrics(pred: torch.Tensor, label: torch.Tensor) -> dict:
    ...
    iou_pred_1_and_label_1 = calculate_iou(
        pred[..., :4], label[..., :4])  # (bs, 7, 7)

    # 5, 6, 7, 8 - bbox 2. 9 - bbox 2 confidence
    iou_pred_2_and_label_2 = calculate_iou(
        pred[..., 5:9], label[..., 5:9])  # (bs, 7, 7)
    # Check if the predicted class labels are correct
    is_class_correct = (pred[..., B*5:] == label[...,
                        B*5:]).all(dim=-1)  # (bs, 7, 7)

    num_correct_for_pred_box_1 = torch.logical_and(
        (iou_pred_1_and_label_1 > 0.5), is_class_correct).sum().item()
    num_correct_for_pred_box_2 = torch.logical_and(
        (iou_pred_2_and_label_2 > 0.5), is_class_correct).sum().item()

    num_incorrect_localization_for_pred_box_1 = torch.logical_and(
        torch.logical_and(does_label_1_exist_for_each_cell,
                          torch.logical_and(0.1 <= iou_pred_1_and_label_1, iou_pred_1_and_label_1 <= 0.5)),
        is_class_correct).sum().item()

    num_incorrect_localization_for_pred_box_2 = torch.logical_and(
        torch.logical_and(does_label_2_exist_for_each_cell,
                          torch.logical_and(0.1 <= iou_pred_2_and_label_2, iou_pred_2_and_label_2 <= 0.5)),
        is_class_correct).sum().item()

    num_incorrect_other_for_pred_box_1 = torch.logical_and(
        (iou_pred_1_and_label_1 > 0.1), ~is_class_correct).sum().item()

    num_incorrect_other_for_pred_box_2 = torch.logical_and(
        (iou_pred_2_and_label_2 > 0.1), ~is_class_correct).sum().item()

    num_incorrect_background_for_pred_box_1 = torch.logical_and(
        (iou_pred_1_and_label_1 <= 0.1), is_class_correct).sum().item()

    num_incorrect_background_for_pred_box_2 = torch.logical_and(
        (iou_pred_2_and_label_2 <= 0.1), is_class_correct).sum().item()

    return {"num_correct": num_correct_for_pred_box_1 + num_correct_for_pred_box_2,
            "num_incorrect_localization": num_incorrect_localization_for_pred_box_1 + num_incorrect_localization_for_pred_box_2,
            "num_incorrect_other": num_incorrect_other_for_pred_box_1 + num_incorrect_other_for_pred_box_2,
            "num_incorrect_background": num_incorrect_background_for_pred_box_1 + num_incorrect_background_for_pred_box_2}
